...
<body>
	<div>
		...
		<div style="display:none">
			<input name="id" id="id" value=${param.id} type="text" class="input-normal control-text">
		</div>
	...
	<div id="btns">
		<div>
			<button onclick="saveContent()">保存html的内容</button>
		</div>
		<div>
    ...
    function saveContent() {
    	...
    	var id = $("#id").val();
    	jQuery.post('${basePath}/sysGiftSpread/saveSysGiftSpreadBody.do',{body:body,id:id},function(data){
    	     if(data){
    	     	 if(data == 'ok'){
					alert("保存成功！");    	     	 
    	     	 }else{
    ...
    $(document).ready(function(e) {
		var id = $("#id").val();
		jQuery.post('${basePath}/sysGiftSpread/getBody.do',{id:id},function(data){
	    	if(data){
	    	 	/* alert(data.body); */
	    	 	//console.log(typeof(UE.getEditor('editor').body.innerHTML));
	    	 	/* console.log("UE.Editor.body.innerHTML:");
	    	 	console.log(UE.Editor.body.innerHTML);
	    	 	console.log("UE.getEditor('editor').body.innerHTML:");
	    	 	console.log(UE.getEditor('editor').body.innerHTML); */
		    	//UE.getEditor('editor').setContent(data.body);
		    	//setContent(data.body); 
		    	if(data.body != null){
			    	//goSetContent(data.body);
			    	console.log("开始");
			    	var res = self.setInterval(function(){
			       		//console.log("开始填充body到UE.getEditor('editor').body.innerHTML");
			       		//console.log("typeof(UE.getEditor('editor').body.innerHTML)是否已存在:"+typeof(UE.getEditor('editor').body.innerHTML));
	...
	function goSetContent (body) {
        var res = self.setInterval(function(){
       		console.log("开始填充body到UE.getEditor('editor').body.innerHTML");
       		console.log("typeof(UE.getEditor('editor').body.innerHTML)是否已存在:"+typeof(UE.getEditor('editor').body.innerHTML));
			if(typeof(UE.getEditor('editor').body.innerHTML) != "undefined"){
				UE.getEditor('editor').setContent(body);
				clearInterval(res);
				console.log("填充完毕，停止setInterval循环");
			}
			console.log("500ms执行一次，本次已结束了");
