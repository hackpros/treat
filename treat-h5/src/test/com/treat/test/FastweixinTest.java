...
import com.navigate.treat.wechat.pojo.WeixinUserList;

public class FastweixinTest {
	private static final org.slf4j.Logger LOG = LoggerFactory.getLogger(FastweixinTest.class);
	...
	public void init() {
		...
		String appid = "wx12cb7e68a206ae20";
		String secret = "45b9b7a0091793bd3a5a22e79052eea9";
	...
	@Test
	public void test() {
		String appid = "wxe2bba7f91807e5b7";
		String secret = "d9f64501a803d34968421877faa0817d";
		ApiConfig config = new ApiConfig(appid, secret);
		TestConfigChangeHandle configChangeHandle = new TestConfigChangeHandle();
		config.addHandle(configChangeHandle);
		// createMenu(config);
		getUserList(config);
		List<WeixinUserList> list=new ArrayList<WeixinUserList>();
		// uploadMedia(config);
	...
	public void oauthGetUserInfo(ApiConfig config) {
		OauthAPI oauthAPI = new OauthAPI(config);
		GetUserInfoResponse response = oauthAPI
				.getUserInfo(
	...
	public void uploadNewsMaterial() {
		MaterialAPI materialAPI = new MaterialAPI(config);
		Article article = new Article("VnzJFSwv05ezhWSlU3kV6fmFYxHXaIHQMxx2SjX87fg", "测试", "测试", "http://www.baidu.com", "测试新闻。无意义",
				"测试新闻。无意义", Article.ShowConverPic.YES);
